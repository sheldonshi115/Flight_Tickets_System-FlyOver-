...
void MainWindow::setIsAdmin(bool isAdmin)
{
    ...
    if (isAdmin) {
        ...
        if (ui->sideBar) {
            ...
            for (QPushButton *btn : sideButtons) {
                if (btn) {
                    btn->setStyleSheet(adminSidebarStyle);
                }
            }
        }
        ...
        if (ui->sideBar) {
            ...
            for (QPushButton *btn : sideButtons) {
                if (btn) {
                    btn->setStyleSheet(userSidebarStyle);
                }
            }
        }
...
void MainWindow::initNotificationSystem()
{
    ...
    connect(&NotificationManager::instance(), &NotificationManager::notificationReceived,
            this, [this](const NotificationItem& item) {
...
void MainWindow::createQuickActionButtons()
{
    ...
    btnQuickFlight->setMinimumSize(150, 110);
    ...
    connect(btnQuickMap, &QPushButton::clicked, this, [this]() {
        // 创建地图可视化窗口
