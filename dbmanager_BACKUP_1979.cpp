...
QVector<MomentItem> DBManager::getAllMoments()
{
    QVector<MomentItem> moments;
    if (!db.isOpen()) return moments;
    ...
    if (!query.exec("SELECT * FROM moments ORDER BY publish_time DESC")) {
        qWarning() << "查询动态失败:" << query.lastError().text();
        return moments;
    }
    ...
    while (query.next()) {
        ...
        item.avatarPath = query.value("avatar_path").toString();
        item.content = query.value("content").toString();
        QString imagesStr = query.value("images").toString();
        if (!imagesStr.isEmpty()) {
            item.images = imagesStr.split(";", Qt::SkipEmptyParts);
        }
...
bool DBManager::addMoment(const MomentItem& item)
{
    ...
    QSqlQuery query(db);
    query.prepare(R"(
        INSERT INTO moments (user_name, avatar_path, content, images, publish_time, like_count, comment_count)
        VALUES (:user_name, :avatar_path, :content, :images, :publish_time, :like_count, :comment_count)
    )");
    query.bindValue(":user_name", item.userName);
    query.bindValue(":avatar_path", item.avatarPath);
    query.bindValue(":content", item.content);
    query.bindValue(":images", item.images.join(";"));
    ...
    if (!query.exec()) {
        qWarning() << "添加动态失败:" << query.lastError().text();
        return false;
    }
    return true;
}
...
bool DBManager::updateMomentLike(int momentId, bool liked)
{
    ...
    QSqlQuery query(db);
    QString sql = liked ? 
        "UPDATE moments SET like_count = like_count + 1 WHERE id = :id" :
        "UPDATE moments SET like_count = GREATEST(like_count - 1, 0) WHERE id = :id";
    query.prepare(sql);
    query.bindValue(":id", momentId);
    
    if (!query.exec()) {
        qWarning() << "更新点赞状态失败:" << query.lastError().text();
        return false;
    }
    return true;
}
...
QString DBManager::getAvatarByNickname(const QString& nickname)
{
    ...
    QSqlQuery query(db);
    query.prepare("SELECT image FROM users WHERE nickname = :nickname");
    query.bindValue(":nickname", nickname);
    
    if (query.exec() && query.next()) {
        return query.value("image").toString();
    }
    return QString();
}
...
bool DBManager::addComment(int momentId, const QString& content, const QString& userName)
{
    ...
    query.prepare(R"(
        INSERT INTO moment_comments (moment_id, user_name, content, create_time)
        VALUES (:moment_id, :user_name, :content, NOW())
    )");
    ...
    // 更新动态的评论数
    query.prepare("UPDATE moments SET comment_count = comment_count + 1 WHERE id = :id");
    query.bindValue(":id", momentId);
    query.exec();
...
QList<Comment> DBManager::getCommentsByMomentId(int momentId)
{
    ...
    if (!db.isOpen()) return comments;
    
    QSqlQuery query(db);
    query.prepare("SELECT * FROM moment_comments WHERE moment_id = :moment_id ORDER BY create_time ASC");
    query.bindValue(":moment_id", momentId);
    ...
    if (query.exec()) {
        while (query.next()) {
            ...
            c.momentId = query.value("moment_id").toInt();
...
bool DBManager::addSystemEmail(const QString& account, const QString& subject, const QString& body)
{
    ...
    query.prepare(R"(
        INSERT INTO system_emails (account, subject, body, is_read, create_time)
        VALUES (:account, :subject, :body, 0, NOW())
    )");
    query.bindValue(":account", account);
    ...
    if (!query.exec()) {
        qWarning() << "添加系统邮件失败:" << query.lastError().text();
        return false;
    }
    
    qDebug() << "系统邮件已添加，账号：" << account << "主题：" << subject;
    return true;
}
...
QList<SystemEmail> DBManager::getSystemEmails(const QString& account)
{
    ...
    QSqlQuery query(db);
    query.prepare("SELECT * FROM system_emails WHERE account = :account ORDER BY create_time DESC");
    query.bindValue(":account", account);
    ...
    if (query.exec()) {
        while (query.next()) {
            ...
            email.account = query.value("account").toString();
        ...
        qWarning() << "获取系统邮件失败:" << query.lastError().text();
    }
...
bool DBManager::markSystemEmailAsRead(int emailId)
{
    ...
    QSqlQuery query(db);
    query.prepare("UPDATE system_emails SET is_read = 1 WHERE id = :id");
    query.bindValue(":id", emailId);
    
    if (!query.exec()) {
        qWarning() << "标记邮件已读失败:" << query.lastError().text();
        return false;
    }
    return true;
}
...
bool DBManager::deleteSystemEmail(int emailId)
{
    ...
    QSqlQuery query(db);
    query.prepare("DELETE FROM system_emails WHERE id = :id");
    query.bindValue(":id", emailId);
