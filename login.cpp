...
// ========== 简单可逆加密（用于记住我密码存储） ==========
// 注意：此处为轻量级可逆混淆，用于在本地配置中保存密码以便自动填充。
// 这不是强加密，生产环境应使用系统凭据保管（如 Windows Credential Store / Keychain）或更安全的加密库。
static QByteArray xorWithKey(const QByteArray &data, const QByteArray &key) {
    QByteArray out;
    out.resize(data.size());
    for (int i = 0; i < data.size(); ++i) {
        out[i] = data[i] ^ key.at(i % key.size());
    }
    return out;
}
...
static QString encryptPassword(const QString &plain) {
    ...
    QByteArray key = QCryptographicHash::hash(QCoreApplication::applicationName().toUtf8(), QCryptographicHash::Sha256);
    QByteArray x = xorWithKey(data, key);
    return QString::fromLatin1(x.toBase64());
}

static QString decryptPassword(const QString &enc) {
    if (enc.isEmpty()) return QString();
    QByteArray b = QByteArray::fromBase64(enc.toLatin1());
    QByteArray key = QCryptographicHash::hash(QCoreApplication::applicationName().toUtf8(), QCryptographicHash::Sha256);
    QByteArray x = xorWithKey(b, key);
    return QString::fromUtf8(x);
}

// ========== ParticlePanel 实现 (==========
ParticlePanel::ParticlePanel(QWidget *parent)
    : QWidget(parent), m_timerId(0), m_gradientOffset(0.0f)
{
    ...
    m_particlePanel(nullptr)
{
    ...
    // 当复选框状态变化时，添加明显的勾选文本反馈（确保在所有样式下用户能看到已选中）
...
void LoginDialog::loadRememberedCredentials()
{
    ...
    QString savedPasswordEnc = settings.value("login/password", "").toString();
    ...
    if (remembered && !savedAccount.isEmpty()) {
        ...
        // 如果保存了加密密码，则尝试解密并填充密码框（注意安全性）
        if (!savedPasswordEnc.isEmpty()) {
            // 解密并填充密码
            QString pwd = decryptPassword(savedPasswordEnc);
            ui->passwordEdit->setText(pwd);
        }

        // 将焦点设置到密码框（若已填充密码可将焦点移到登录按钮）
        if (ui->passwordEdit->text().isEmpty()) ui->passwordEdit->setFocus();
    }
}
...
void LoginDialog::saveCredentials()
{
    ...
    // 为了实现自动填充密码，这里将密码进行轻量加密后存储到本地配置
    // 注意：本地存储密码存在安全风险，生产环境应使用更安全的凭据存储方案
    QString encrypted = encryptPassword(ui->passwordEdit->text());
    settings.setValue("login/password", encrypted);
